...
.controller('jskfjCtrl', ['$scope', '$http', '$q', 'jskfjFactory', function ($scope, $http, $q, jskfjFactory) {

    // Import variables and functions from service
    $scope.data = jskfjFactory.data;
	$scope.items = jskfjFactory.items;
	$scope.detailsItem = jskfjFactory.detailsItem;
	
    // initialize the service
    jskfjFactory.init($scope);

	// Show main view in the first column
	$scope.portalHelpers.showView('jskfjMain.html', 1);
	
	// This function gets called when user clicks an item in the list
	$scope.showDetails = function(item){
		// Make the item that user clicked available to the template
		$scope.detailsItem.value = item;		
		$scope.portalHelpers.showView('jskfjDetails.html', 2);
	}
	
}])
// Factory maintains the state of the widget
.factory('jskfjFactory', ['$http', '$rootScope', '$filter', '$q', function ($http, $rootScope, $filter, $q) {
...
.directive('jskfjDirectiveName', ['$http', function ($http) {
	return {
...
.filter('jskfjFilterName', function () {
